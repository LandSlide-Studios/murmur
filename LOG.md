...
### Same day - three regressions the tier 4+5 gate found in the tier 4+5 fixes

**The silence detector became CUMULATIVE.** Requiring a sustained burst to clear
the counter meant a burst that never reached 0.30s cleared nothing, so every gap
was banked across the whole session. Someone reading a list aloud was cut off
**122 seconds into an active dictation**; ordinary speech with 50ms
stop-consonant closures at 450 seconds.

Burst length was the wrong discriminator entirely, and measuring it says so:
real words are SHORT (a list read aloud is 0.25s on, 0.70s off) while room noise
can be LONG (a door is 350ms). So the rule cut off the speaker AND let the empty
room run for an hour -- both promises broken by the thing meant to fix one.

Duty cycle separates them cleanly. Speech is a quarter to four fifths voiced; a
room with an occasional noise is well under one percent. Any sound clears the
continuous-silence counter again, as before, and a decaying average of how much
of the recent past carried sound catches the forgotten session.

| pattern | duty | result |
|---|---|---|
| reading a list aloud, 0.25s / 0.70s | 26% | runs 900s |
| slow speaker, 0.28s / 0.25s | 53% | runs 900s |
| ordinary speech, 50ms closures | 80% | runs 900s |
| long thinking pauses, 1.2s / 1.8s | 39% | runs 900s |
| a door every 89s | 0.4% | stops at 124s |
| a fan cycling every 20s | 2.5% | stops at 198s |
| genuinely silent | 0% | stops at 90s |

**Stopping the overlay's timer broke two things that ran on that clock.**
`off` was the one path that changes state without restarting it, so the fade
never ran and a translucent always-on-top capsule stayed on the bezel
permanently -- pausing the hotkeys from the tray hits exactly this. And the
adaptive gain decays only in the idle paths, which run on the same timer, so
after a loud dictation it froze at whatever that left and the next session's
first word read low. That is precisely the failure the decay was added for. It
now forgets outright when the clock stops, since a decay measured in frames has
no clock to run on.
